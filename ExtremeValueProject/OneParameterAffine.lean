...
noncomputable def AffineIncrEquiv.subGoupOfIndex₀' (β : ℝ) :
    Subgroup AffineIncrEquiv :=
  Subgroup.map (AffineIncrEquiv.homOfIndex₀ β) ⊤

/-- The one-parameter subgroup of `AffineIncrEquiv` consisting of elements `Aₛ` of the form
`Aₛ x = x + s`, where `s ∈ ℝ`. -/
noncomputable def AffineIncrEquiv.subGoupOfIndex₀ :
    Subgroup AffineIncrEquiv :=
  Subgroup.map (AffineIncrEquiv.homOfIndex₀ 1) ⊤

@[simp] lemma AffineIncrEquiv.subGoupOfIndex₀'_eq_of_ne_zero {β : ℝ} (hβ : β ≠ 0) :
    AffineIncrEquiv.subGoupOfIndex₀' β = AffineIncrEquiv.subGoupOfIndex₀ := by
  sorry -- **Issue 43?** (?)

@[simp] lemma AffineIncrEquiv.subGoupOfIndex₀'_eq_bot :
    AffineIncrEquiv.subGoupOfIndex₀' 0 = ⊥ := by
  sorry -- **Issue 43** (?)

@[simp] lemma AffineIncrEquiv.mem_subGoupOfIndex₀_of_no_fixed_point (A : AffineIncrEquiv)
...
noncomputable def AffineIncrEquiv.subGoupOfIndex (α c : ℝ) :
    Subgroup AffineIncrEquiv :=
  Subgroup.map (AffineIncrEquiv.homOfIndex α c) ⊤

@[simp] lemma AffineIncrEquiv.subGoupOfIndex_eq_bot (c : ℝ) :
    subGoupOfIndex 0 c = ⊥ := by
  sorry -- **Issue 44** (?)

@[simp] lemma AffineIncrEquiv.fixed_point_of_mem_subGoupOfIndex (A : AffineIncrEquiv)
    {α c : ℝ} (hA : A ∈ subGoupOfIndex α c):
    A c = c := by
  sorry -- **Issue 44** (?)

@[simp] lemma AffineIncrEquiv.mem_subGoupOfIndex_iff_fixed_point (A : AffineIncrEquiv)
    {α : ℝ} (hα : α ≠ 0) (c : ℝ) :
    A ∈ subGoupOfIndex α c ↔ A c = c := by
  sorry -- **Issue 44** (?)
